...
function getInputGroupButton(button) {
  return <div className="input-group-btn">{button}</div>;
}

function getInputGroup(children) {
  return React.createElement.apply(
    null,
    ['div', { className: 'input-group' }].concat(children)
  );
}
...
function create(overrides = {}) {
  ...
  textbox.renderHiddenTextbox =
    ...
    function renderHiddenTextbox(locals) {
      return (
        <input type="hidden" value={locals.value} name={locals.attrs.name} />
      );
    };
  ...
  textbox.renderInputGroup =
    ...
    function renderInputGroup(input, locals) {
      return getInputGroup([
        locals.config.buttonBefore
          ? getInputGroupButton(locals.config.buttonBefore)
          : null,
  ...
  textbox.renderInput =
    ...
    function renderInput(locals) {
      return <input {...locals.attrs} key={locals.path.join('-')} />;
    };
